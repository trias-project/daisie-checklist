...
`input_taxon` has one fields that can't be used for mapping of the Taxon Core:
 - `ecofunct_group`
...
## From input_distribution

`input_distribution` has several fields that can't be used for mapping of the Distribution Extension:
- `population_status`
...
1. Save information on population status
...
2. Remove records for which `population_status` is empty:

```{r}
population_status %<>% filter(!is.na(population_status))
```

3. Add `sourceid` (which is empty, but required field for mapping description extension)

```{r}
population_status %<>% mutate(sourceid = "") 
```

4. Export as `population_status`

```{r}
write_csv(population_status, "../data/interim/population_status.csv", na = "")
```

5. Save information on current distribution
...
6. Remove records for which `current_distribution` is empty:

```{r}
current_distribution %<>% filter(!is.na(current_distribution))
```

7. Add `sourceid` (which is empty, but required field for mapping description extension)

```{r}
current_distribution %<>% mutate(sourceid = "") 
```

8. Export as `current_distribution`

```{r}
write_csv(current_distribution, "../data/interim/current_distribution.csv", na = "")
```

9. Save information on region of first record:
...
10. Remove records for which `region_of_first_record` is empty:

```{r}
region_of_first_record %<>% filter(!is.na(region_of_first_record))
```

11. Add `sourceid` (which is empty, but required field for mapping description extension)

```{r}
region_of_first_record %<>% mutate(sourceid = "") 
```

12. Export as `region_of_first_record`

```{r}
write_csv(region_of_first_record, "../data/interim/region_of_first_record.csv", na = "")
```
