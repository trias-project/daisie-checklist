...
```{r}
# Raw data
input_distribution <- read_csv(here::here("data", "raw", "input_distribution.csv"), col_types = col_types)
input_donor_area <- read_csv(here::here("data", "raw", "input_donor_area.csv"), col_type = col_types)
input_pathways <- read_csv(here::here("data", "raw", "input_pathways.csv"), col_type = col_types)
input_impact <- read_csv(here::here("data", "raw", "input_impact.csv"), col_type = col_types)

# Preprocessed interim data
distribution_sources <- read_csv(here::here("data", "interim", "distribution_sources.csv"), col_types = col_types)
remove_taxa <- read_csv(here::here("data", "interim", "remove_taxa.csv"), col_types = col_types)
```
...
2. Show replacement value:

```{r}
remove_taxa %>% 
  filter(idspecies == "52845") %>% 
  select(replacement_idspecies)
```
...
```{r}
distribution %<>% mutate(dwc_countryCode = 
...
not_date_indices <- !str_detect(distribution$start_year, 
                               "[0-9]{4}-[0-9]{2}-[0-9]{2}") & !is.na(distribution$start_year)
...
not_date_indices <- !str_detect(distribution$end_year, 
                               "[0-9]{4}-[0-9]{2}-[0-9]{2}") & !is.na(distribution$end_year)
