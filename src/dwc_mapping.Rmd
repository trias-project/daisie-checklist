...
In the distribution extension, we need the specific link bew `source` to a specific `id_sp_region` x `field_name` combination (see below). We extract this information here and save it as the dataframe `distribution_sources`:
...
Note that coastal information is not alway s provided (`NA`). For these records, we will not integrate coastal information into `locationID` and `locality`.
