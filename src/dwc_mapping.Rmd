...
These species require some further inspection.
...
Information for `taxonRank` is provided in the field `subtaxon_rank`. However, this is only for the subtaxon level. The information is also provided by the GBIF nameparser function

Inspect values:

```{r}
taxon %>% 
  group_by(sp_subtaxon_rank) %>% 
  summarize(records = n())
```

SOme of these taxon ranks are erroneous:
- `Cytosporina sp.`
- `Crous`

`Cytosporina sp.` is the taxon_rank of 
