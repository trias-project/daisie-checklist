...
```{r}
col_types <- cols(
  id_species = col_character(),
  genus = col_character(),
  species = col_character(),
  authority = col_character(),
  subtaxon = col_character(),
  subtaxon_rank = col_character(),
  subtaxon_authority = col_character(),
  taxon_group = col_character(),
  phylum = col_character(),
  class = col_character(),
  ordo = col_character(),
  family = col_character(),
  ecofunct_group = col_character(),
  sourceid = col_character(),
  name = col_character(),
  language = col_character(),
  longref = col_character(),
  shortref = col_character(),
  url = col_character(),
  id_sp_region = col_character(),
  code_region = col_character(), 
  region_country = col_character(),
  system_country = col_character(),
  code_coast = col_character(), 
  region_coast = col_character(),
  system_coast = col_character(),
  abundance = col_character(), 
  population_status = col_character(), 
  region_of_first_record = col_character(), 
  current_distribution = col_character(),
  start_year = col_character(),
  end_year = col_character(),
  species_status = col_character(),
...
pathways <- read_csv("../data/raw/input_pathways.csv")
vectors  <- read_csv("../data/raw/input_vectors.csv")
habitat <- read_csv("../data/raw/input_habitat.csv")
native_range <- read_csv("../data/raw/input_native_range.csv")
donor_area <- read_csv("../data/raw/input_donor_area.csv")
impact <- read_csv("../data/raw/input_impact.csv")
```

# Pre-processing

The file `literature_references` contains detailed information about the consulted sources for DAISIE. We use this information to assign a full reference to each record coupled to a `sourceid`. These full references are assigned to the fields `taxonRemarks` (in the taxon core) or `source` (for distribution, vernacular names and description extension). 
 
The full reference integrates information from different fields in `literature_references`:
- `shortref` (ab) and/or
- `longref` and/or
- `url`

1. Clean references somewhat:
...
Several `longref` values start with a number (e.g. `1`, `1, 28`) These are not really valuable for the dataset and can be removed:

```{r}
literature_references %<>% mutate(longref = str_replace(longref, "^[0-9]", NA_character_))
```

Inspection of `shortref` values:
...
COMPLETE: The information in `url` is, in most cases, wrong: it contains scientific names rather than a real url. We remove these scientific names from this field:

```{r}
literature_references %<>%  mutate(url = if_else(str_detect(url, pattern = "^http"), url, NA_character_))
```

Amount of sources for which 
...
```{r}
kable(overview_references)
```

We translate this to the new field `source`
...
In both the taxon core, vernacular name extension and description extension, the information in `source` from `literature_references` is directly linked to `sourceid` in the respective dataset.

In the case of the distribution extension, a source (in `literature_references`) is attributed to a specific `field_name` (in `literature_references `, refering to a column name in the `input_distriution`) x `id_sp_region` (in both `literature_references` and `input_distribution`, i.e. the id of a taxon in a particular region) combination. We save this information in a separate dataframe `distribution_sources` here:

```{r}
distribution_sources <- literature_references %>% 
...
taxon %<>% 
  ...
  mutate(authority = if_else(is.na(authority), "", authority)) %>% 
  mutate(subtaxon = if_else(is.na(subtaxon), "", subtaxon)) %>% 
  mutate(subtaxon_rank = if_else(is.na(subtaxon_rank), "", subtaxon_rank)) %>% 
  mutate(subtaxon_authority = if_else(is.na(subtaxon_authority), "", subtaxon_authority))
```
...
Not all variables can be mapped to DwC terms in the distribution extension. We save the information in a separate dataframes and map the information in the description extension:
...
These codes originate from different sources (mapped to `locationID`), contained respectively in:
- `system_country`
- `system_coast` 
...
This is how we will map information about locations:
...
Some information in the description extension (e.g. for `impact on use` or `impact on ecolgy`) is a property of a species in a particular _region_, rather than a property of a species as a whole. This is indicated by linking the description infomration with the ID `id_sp_region`. 
To emphasize that this description record is linked to a species in a particular region, we want to add the `locationID` to the description information:
...
We thus generate a dataframe here, linking `id_sp_region` with `locationID`:
...
In the distribution extension, `source` information is extracted from `distribution_sources`. We link this information to `distribution` by using a combination between `id_sp_region` and `field_name`, with the content of `field_name` referring to the specific column names in `input_distribution`
